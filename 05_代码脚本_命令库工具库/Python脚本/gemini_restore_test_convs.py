"""
Restore test-prefixed Gemini conversations: remove [TEST]/[TST] prefix.
"""
import asyncio, re
...
async def rename_conv(page, conv_id, new_title):
    """UI-driven rename. Returns True if success."""
    ...
    ok = await page.evaluate("""(newTitle) => {
...
async def main():
    async with async_playwright() as p:
        ...
        # Find test-prefixed
        targets = [(c["id"], c["title"]) for c in convs
                   if c["title"].startswith("[TEST]") or c["title"].startswith("[TST]") or c["title"].startswith("[DEV] Test")]

        if not targets:
            print("No test-prefixed convs found")
            await browser.close()
            return

        print(f"Found {len(targets)} test-prefixed convs")
        restored = 0

        for conv_id, current_title in targets:
            # Strip prefix
            new_title = re.sub(r'^\[(?:TEST|TST|DEV)\]\s*', '', current_title).strip()
            # Remove doubled text
            midpoint = len(new_title) // 2
            if midpoint > 5 and new_title[:midpoint] == new_title[midpoint:]:
                new_title = new_title[:midpoint]

            print(f"\nRestore: '{current_title[:50]}' -> '{new_title[:50]}'")
            ok = await rename_conv(page, conv_id, new_title)
            print(f"  {'OK' if ok else 'FAIL'}")
            if ok: restored += 1

        print(f"\nDone: {restored}/{len(targets)} restored")
        await browser.close()
